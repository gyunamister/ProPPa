...
pattern_load_eocpn_button_title = "load enhanced model".title()
pattern_design_button_title = "edit new".title()
pattern_add_button_title = "add new".title()
pattern_refresh_button_title = "reset".title()
...
loading_buttons = [
    button(pattern_load_eocpn_button_title, show_title_maker, show_button_id),
]
    ...
    dbc.Col(
        dbc.Card(
            [
                dbc.Row(
                    [
                        dbc.Col(html.H2(f"Object Relation Patterns",
                                className="card-title")),
                        dbc.Col(
                            dbc.Button(
                                'Add', 'add-object-relation-edge', className="mb-3")
                        )
                    ], align='center'
                ),
                html.Hr(),
            ...
            [
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col(
                            dcc.Input(id="formula-threshold", type="number", placeholder=0, style={'width': '70%'}), width=4
                        ),
                    ],
                    align='center'
                ),
...
pattern_graph_view = dbc.Card(
    children=[
        html.H2('Show existing patterns:', id='pattern-graph-label'),
...
def refresh_patterns(n_load, value, perf_jobs):
    ctx = dash.callback_context
    if not ctx.triggered:
        button_id = 'No clicks yet'
        button_value = None
    ...
    if button_id == show_button_id(pattern_load_eocpn_button_title):
        ...
        activities = [t.name for t in ocpn.transitions if t.silent == False]
        ...
        return dropdown_activities, dropdown_activities, dropdown_object_types, dropdown_activities, dropdown_activities, dropdown_frequency, dropdown_aggregations, dropdown_object_types
...
@ app.callback(
    ...
    Input('add-object-relation-edge', 'n_clicks'),
...
def start_editing_patterns(n_edit, n_add, n_cf, n_or, n_perf, n_name_ok, n_name_cancel, edit_pattern_graph, pg_name, cf_source, cf_target, cf_label, or_source, or_target, or_label, formula_agg, formula_diag, formula_obj, formula_comp, formula_thre, perf_target, value, perf_jobs):
    ctx = dash.callback_context
    if not ctx.triggered:
        button_id = 'No clicks yet'
        button_value = None
...
def update_edit_pattern_graph(edit_pattern_graph):
    nodes = set()
    pg_edges = []
    pg_nodes = []
    for elem in edit_pattern_graph['cf_edges']:
        ...
        pg_edges.append(
            {"data": {"source": source, "target": target, "label": elem['label']}})
    ...
    for elem in edit_pattern_graph['perf_edges']:
        if elem['formula_agg'] is not None and elem['formula_obj'] is not None:
            source = f"{elem['formula_agg']} {elem['formula_diag']} {elem['formula_obj']} {elem['formula_comp']} {elem['formula_thre']}"
        elif elem['formula_agg'] is None and elem['formula_obj'] is not None:
            source = f"{elem['formula_diag']} {elem['formula_obj']} {elem['formula_comp']} {elem['formula_thre']}"
        elif elem['formula_agg'] is not None and elem['formula_obj'] is None:
            source = f"{elem['formula_agg']} {elem['formula_diag']} {elem['formula_comp']} {elem['formula_thre']}"
        else:
            raise ValueError(
...
def add_pattern(n_refresh, n_load, n_add, value, perf_jobs, pattern_jobs, edit_pattern_graph):
    ctx = dash.callback_context
    if not ctx.triggered:
        button_id = 'No clicks yet'
        button_value = None
    ...
    elif button_id == show_button_id(pattern_load_eocpn_button_title):
        user = request.authorization['username']
        log_hash, date = read_global_signal_value(value)
